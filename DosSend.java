...
public class DosSend {
   ...
   public void writeNormalizeWavData(){
    try {
        // Parcours de toutes les valeurs modulées dans les données audio
        for (double datum : dataMod) {
            // Normalisation de la valeur à la plage des valeurs autorisées du format audio
                short normalized = (short) (datum * MAX_AMP);

            // Écriture des données normalisées dans le fichier WAV par blocs
            // Chaque bloc correspond à la taille en octets du format audio divisée par 8
                writeLittleEndian(normalized, FMT / 8, outStream);
            }
        } catch (Exception e) {
        // Gestion basique des erreurs en cas de problème lors de l'écriture
            System.out.println("Erreur d'écriture");
        }
    }
    ...
    // Calcul du nombre d'échantillons par symbole
    int samplesPerSymbol = FECH / BAUDS;
    // Calcul du nombre total d'échantillons pour toutes les données
    int totalSamples = bits.length * samplesPerSymbol;
    
    // Initialisation du tableau pour stocker les données modulées
    dataMod = new double[totalSamples];

    // Parcours des bits d'entrée
    for (int i = 0; i < bits.length; i++) {
        // Calcul de l'amplitude en fonction du bit (1 ou 0) et de l'amplification
        double amplitude = (bits[i] == 1) ? amplification : -amplification;
        // Calcul du décalage de phase pour le signal
        double phaseShift = 2 * Math.PI * FP / FECH;

        // Génération des échantillons pour chaque bit
        for (int j = 0; j < samplesPerSymbol; j++) {
            // Calcul de l'index dans le tableau de données modulées
            int index = i * samplesPerSymbol + j;
            // Modulation : amplitude * sin(phaseShift * temps)
            dataMod[index] = amplitude * Math.sin(phaseShift * j);
        }
    }
}
